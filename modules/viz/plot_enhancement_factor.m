...
hline = yline(ax, 1, '--k', 'No Enhancement', ...
...
% Theme-aware styling for reference line
try
    fg = ax.XColor; 
    set(hline, 'Color', fg);
    if isprop(hline, 'Label') && isgraphics(hline.Label)
        hline.Label.Color = fg;
    end
catch
end
grid(ax, 'on');
...
% Theme-aware annotation box
fg = ax.XColor; bg = get(fig, 'Color');
