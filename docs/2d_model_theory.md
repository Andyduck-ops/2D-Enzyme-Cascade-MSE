...
**布朗步进公式**：$\Delta\mathbf{r} = \sqrt{2D\,\Delta t}\,\boldsymbol{\eta}$，其中 $\boldsymbol{\eta} \sim \mathcal{N}(\mathbf{0}, \mathbf{I}_2)$
...
**反应概率公式**：$p = 1 - \exp\bigl(-k_{\text{eff}}\,\Delta t\bigr)$，其中 $k_{\text{eff}} = k_{\text{cat}}\bigl(1 - \text{inhibition}\bigr)$

判定：采样 $u \sim \mathcal{U}(0,1)$，若 $u < p$，则发生反应事件（$\mathrm{S}\rightarrow\mathrm{I}$ 或 $\mathrm{I}\rightarrow\mathrm{P}$）。事件坐标：在相遇对（酶-底物）局部附近采样并记录，用于事件热力图。
...
**蒙特卡洛统计公式**：
- $\hat{\mu} = \dfrac{1}{M} \sum_{m=1}^{M} P_m$
