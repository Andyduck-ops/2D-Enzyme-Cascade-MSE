...
- 2D 抽象：用环区 $$ [r_p, r_p+f_t] $$ 近似中心颗粒周围的表面薄膜，酶在 MSE 模式固定于环区，bulk 模式则在盒域均匀分布。默认参数体现强扩散对比（如 $$ D_{\mathrm{film}} = 10 $$ vs $$ D_{\mathrm{bulk}} = 1000 $$ nm²/s）与适中的膜厚（$$ f_t = 5 $$ nm），见 [modules/config/default_config.m](modules/config/default_config.m)。
